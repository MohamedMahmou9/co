...
api_key = '7e65d999cdb22f661fc01fdd8ac21b6e'
client = ElevenLabs(api_key=api_key)

# إعداد بوت تليجرام
TELEGRAM_TOKEN = '6815820225:AAE2xHaZF46RJt0JIljANkfiWBNC4D1kXBM'
...
# تجزئة النص إلى أسطر متعددة إذا كان طويلاً
def wrap_text(text, font_size, max_width):
    import textwrap
    wrapped_lines = textwrap.wrap(text, width=int(max_width / font_size))
    return "\n".join(wrapped_lines)
...
def create_video_clip(quote, author, image_path, index, vertical):
    ...
    # إعداد كليب النص
    fontsize = 40 if vertical else 60  # تصغير حجم الخط إذا كان الفيديو عمودي
    text = f"{quote} \n \n {author}"
    
    # تجزئة النص إذا كان طويلاً
    wrapped_text_content = wrap_text(text, fontsize, image_clip.size[0])
    
    text_clip = TextClip(wrapped_text_content, fontsize=fontsize, color='white', bg_color='transparent', size=image_clip.size, method='caption')
